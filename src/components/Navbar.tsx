...
import { Printer, LogOut, User, Menu, X, LayoutDashboard, Layers, Crown, ChevronDown, ShieldCheck } from 'lucide-react';
...
export default function Navbar() {
  const { usuario, logout, impersonando, voltarParaAdmin } = useAuth();
  ...
  return (
    <>
      {impersonando && (
        <div className="sticky top-0 z-50 w-full bg-amber-400 text-amber-900 px-4 py-2 flex items-center justify-between text-sm font-semibold shadow-md">
          <div className="flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 shrink-0" />
            Visualizando como <strong>{usuario?.nome}</strong>
          </div>
          <button
            onClick={() => void voltarParaAdmin()}
            className="flex items-center gap-1.5 bg-amber-900/10 hover:bg-amber-900/20 px-3 py-1 rounded-lg transition font-bold"
          >
          ...
          <div className="md:hidden border-t border-white/30 bg-white/80 backdrop-blur-md px-4 py-4 space-y-2">
            {usuario ? (
              <>
                ...
                {impersonando && (
                  <button
                    onClick={() => { void voltarParaAdmin(); setMenuAberto(false); }}
                    className="flex items-center gap-2 py-2 text-sm font-semibold text-amber-700"
                  >
                ...
                <button
                  onClick={() => { void handleLogout(); setMenuAberto(false); }}
